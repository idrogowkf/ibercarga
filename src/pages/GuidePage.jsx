...
import { getGuidesByLanguage } from '../data/guides';
...
export default function GuidePage({ page }) {
  ...
  const relatedGuides = getGuidesByLanguage(page.language).filter((guide) => guide.path !== page.path).slice(0, 3);
  return <div data-layout="v12" className="v12-shell">
    ...
    <main>
      ...
      <article>
        <header className="articleVisualHero"><div className="articleHeroImage" style={{ backgroundImage: `url('${page.image}')` }} aria-hidden="true" /><div className="wrap articleHeroContent"><p className="eyebrow">{page.eyebrow}</p><h1 className="display heroTitle">{page.heading}</h1><p className="heroDesc">{page.intro}</p><p className="articleMeta"><Link to={page.author.path}>{page.author.heading}</Link><span>{english ? 'Published' : 'Publicado'} {page.publishedDate}</span><span>{english ? 'Reviewed' : 'Revisado'} {page.reviewedDate}</span></p></div></header>
