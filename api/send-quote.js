...
module.exports = async (req, res) => {
    ...
    try {
        ...
        const FROM_EMAIL =
            process.env.FROM_EMAIL || "Ibercarga <no-reply@ibercarga.com>"; // remitente de la empresa
        const ADMIN_EMAIL =
            process.env.ADMIN_EMAIL || "transporte@ibercarga.com";           // tu buzón empresa
      ...
      <table style="font-family:system-ui,-apple-system,Segoe UI,Roboto,sans-serif;width:100%;max-width:640px;margin:auto;border-collapse:collapse">
        ...
        <tr>
          <td style="background:#0d47a1;color:#fff;padding:16px 20px;border-radius:12px">
            ...
            <div style="opacity:.9;font-size:14px">Gracias, ${escapeHtml(nombre)}. Hemos recibido tu petición y te contactaremos muy pronto.</div>
          </td>
        </tr>
        ...
        <tr>
          <td style="color:#666;font-size:12px;text-align:center">
            Ibercarga · +34 624 473 123 · ibercarga.com
          </td>
        </tr>
      </table>
    `;
      ...
      <div style="font-family:system-ui,-apple-system,Segoe UI,Roboto,sans-serif;max-width:720px;margin:auto">
        ...
        <p style="font-size:13px;color:#444;margin-top:12px">Si respondes a este correo, llegará al cliente (Reply-To).</p>
      </div>
    `;

        // 1) Email a la empresa (tú)
        const adminSend = await resend.emails.send({
            from: FROM_EMAIL,
            to: [ADMIN_EMAIL],
            subject: `Nueva solicitud • ${origen} → ${destino} • ${tipo}`,
            html: adminHtml,
            reply_to: email // responder va al cliente
        });

        // 2) Email al cliente (acuse)
        ...
        return json(res, 200, {
            ok: true,
            adminId: adminSend?.id || null,
            clientId: clientSend?.id || null
        });
    } catch (err) {
